...
def mapear_vale_gp(
    ...
    if pontos_g is None:
        ...
        # G=100/P=77 é a configuração real e fixa do sistema desde 2026-07-31
        # (antes G=16/P=40, trocado a pedido do usuário — ver ARQUITETURA.md)
        # e já cai naturalmente nesta grade (P proporcional de G=100 já dá 77),
        # então o Mapa continua comparando a config de produção direto contra
        # a grade, sem precisar de nenhum ponto extra nem caso especial.
        pontos_g = [80, 100, 120, 140, 160, 200, 250]
    ...
    for idx_g, g in enumerate(pontos_g):
        ...
        if g == 100:
            # Configuração real fixa do sistema desde 2026-07-31 — o P
            # proporcional (77) já coincide com o real, sem precisar de
            # caso especial (diferente do antigo G=16, onde o proporcional
            # daria 12 e o robô usava 40 de verdade).
            nome = "G=100/P=77 (configuração real do sistema)"
        else:
            nome = f"G={g}/P={p}"
